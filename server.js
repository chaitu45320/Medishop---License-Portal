...
function hmacSeg(data, len) {
  ...
  let r = '';
  for (let i = 0; i < b.length && r.length < len; i++) r += CHARS[b[i] % CHARS.length];
  return r;
}
...
function genKey(type) {
  const tc = type === 'full' ? 'FULL' : 'TRAL', tf = type === 'full' ? 'F' : 'T';
  const s1 = randSeg(tf), s2 = randSeg(), s3 = hmacSeg(s1+'-'+s2, 6);
  return PREFIX+'-'+tc+'-'+s1+'-'+s2+'-'+s3;
}
...
function daysLeft(at, type) {
  if (type === 'full') return null;
  if (!at) return TRIAL_DAYS;
  return Math.max(0, Math.ceil(((at + TRIAL_DAYS*86400000) - Date.now()) / 86400000));
}
function trialExpired(at, type) {
  if (type === 'full' || !at) return false;
...
function enrich(kh, kr, db) {
  try {
    ...
    return {
      key_hash: kh, key_display: kr.key||'', email: kr.email||'', type: kr.type||'trial',
      max_devices: kr.maxDevices||MAX_DEV, is_active: kr.isActive ? 1 : 0,
      issued_at: kr.issuedAt||0, notes: kr.notes||'', active_devices: active.length,
      ...
      activations: acts.map(d => ({ deviceId: d.deviceId||'', deviceName: d.deviceName||'Unknown',
        activatedAt: d.activatedAt||0, lastSeen: d.lastSeen||0, isRevoked: !!d.isRevoked }))
    };
  } catch (e) {
    return { key_hash: kh, key_display: kr.key||kh, email: kr.email||'', type: kr.type||'trial',
      max_devices: MAX_DEV, is_active: 0, issued_at: 0, notes: '', active_devices: 0,
      fail_count: 0, daysLeft: 0, isExpired: false, activations: [] };
  }
}
...
app.post('/api/activate', (req, res) => {
  ...
  return res.json({ success:true, token, type:kr.type, daysLeft:daysLeft(now,kr.type), activatedAt:now, email:em,
    message: kr.type === 'full' ? 'Full license activated!' : 'Trial activated — '+daysLeft(now,kr.type)+' days remaining.' });
});
...
app.get('/health', (req,res) => {
  const db = loadDB(), keys = Object.values(db.keys||{});
  res.json({ status:'ok', version:'4.0', keys:keys.length, db:_pg?'postgresql':'json', port:PORT });
});

// ── Admin API ─────────────────────────────────────────────────────────────────
app.get('/admin/dashboard', adminAuth, (req,res) => {
  try {
    const db = loadDB(), keys = Object.values(db.keys||{}), today = Date.now()-86400000;
    res.json({ success:true, stats:{ totalKeys:keys.length, fullKeys:keys.filter(k=>k.type==='full').length,
      trialKeys:keys.filter(k=>k.type==='trial').length, activeKeys:keys.filter(k=>k.isActive).length,
      totalDevices:keys.reduce((n,k)=>n+Object.values(k.activations||{}).filter(d=>!d.isRevoked).length,0) },
      todayActivations:(db.logs||[]).filter(l=>l.action==='activate'&&l.result==='ok'&&l.ts>today).length });
  } catch(e) { res.status(500).json({ success:false, error:e.message }); }
});
app.get('/admin/keys', adminAuth, (req,res) => {
  try {
    const db = loadDB();
    res.json({ success:true, keys:Object.entries(db.keys||{}).map(([h,k])=>enrich(h,k,db)).sort((a,b)=>b.issued_at-a.issued_at) });
  } catch(e) { res.status(500).json({ success:false, error:e.message }); }
});
app.post('/admin/keys/generate', adminAuth, (req,res) => {
  try {
    ...
    const key = genKey(type), kh = hashKey(key), db = loadDB();
    if (!db.keys) db.keys = {};
    ...
    saveDB(db);
    res.json({ success:true, key, type, email:email.trim().toLowerCase(), maxDevices:parseInt(maxDevices)||MAX_DEV });
  } catch(e) { res.status(500).json({ success:false, error:e.message }); }
});
app.post('/admin/keys/import', adminAuth, (req,res) => {
  try {
    ...
    const key = licenseKey.trim().toUpperCase(), fmt = validateFmt(key);
    if (!fmt.ok) return res.status(400).json({ success:false, error:'Invalid key: '+fmt.reason });
    const db = loadDB(), kh = hashKey(key);
    if (!db.keys) db.keys = {};
    if (db.keys[kh]) return res.status(409).json({ success:false, error:'Key already registered.' });
    db.keys[kh] = { key, email:email.trim().toLowerCase(), type, maxDevices:parseInt(maxDevices)||MAX_DEV, isActive:true, issuedAt:Date.now(), notes:notes||'', activations:{} };
...
app.post('/admin/keys/:hash/revoke', adminAuth, (req,res) => {
  try { const db=loadDB(); if(!db.keys?.[req.params.hash]) return res.status(404).json({success:false,error:'Not found'}); db.keys[req.params.hash].isActive=false; dbLog(db,req.params.hash,'',req.ip||'','admin_revoke','ok',''); saveDB(db); res.json({success:true,message:'Revoked.'}); }
  catch(e) { res.status(500).json({success:false,error:e.message}); }
});
app.post('/admin/keys/:hash/restore', adminAuth, (req,res) => {
  try { const db=loadDB(); if(!db.keys?.[req.params.hash]) return res.status(404).json({success:false,error:'Not found'}); db.keys[req.params.hash].isActive=true; dbLog(db,req.params.hash,'',req.ip||'','admin_restore','ok',''); saveDB(db); res.json({success:true,message:'Restored.'}); }
  catch(e) { res.status(500).json({success:false,error:e.message}); }
});
app.post('/admin/devices/:hash/transfer-all', adminAuth, (req,res) => {
  try { const db=loadDB(); if(!db.keys?.[req.params.hash]) return res.status(404).json({success:false,error:'Not found'}); db.keys[req.params.hash].activations={}; dbLog(db,req.params.hash,'',req.ip||'','transfer_all','ok',''); saveDB(db); res.json({success:true,message:'All slots freed.'}); }
  catch(e) { res.status(500).json({success:false,error:e.message}); }
});
app.post('/admin/verify', adminAuth, (req,res) => {
  try {
    ...
    const fmt = validateFmt(key);
    if (!fmt.ok) return res.status(400).json({success:false,error:'Invalid: '+fmt.reason});
    const db = loadDB(), kh = hashKey(key), kr = db.keys?.[kh];
    if (!kr) return res.status(404).json({success:false,error:'Key not found.'});
    const acts=Object.values(kr.activations||{}), active=acts.filter(d=>!d.isRevoked);
    const firstAt=active.length>0?Math.min.apply(null,active.map(d=>d.activatedAt||0)):null;
    res.json({success:true,type:kr.type,email:kr.email,daysLeft:daysLeft(firstAt,kr.type),
      isExpired:trialExpired(firstAt,kr.type),isActive:!!kr.isActive,deviceCount:active.length,
      maxDevices:kr.maxDevices||MAX_DEV,keyHash:kh,
      devices:acts.map(d=>({deviceId:d.deviceId||'',deviceName:d.deviceName||'Unknown',activatedAt:d.activatedAt||0,lastSeen:d.lastSeen||0,isRevoked:!!d.isRevoked}))});
  } catch(e) { res.status(500).json({success:false,error:e.message}); }
});
app.get('/admin/suspicious', adminAuth, (req,res) => {
  try {
    const db=loadDB(), fm={};
    (db.logs||[]).filter(l=>l.result&&l.result.startsWith('fail')).forEach(l=>{
      const h=l.kh||'unknown'; if(!fm[h]) fm[h]={kh:h,count:0,last:0,ips:new Set()};
      fm[h].count++; if(l.ts>fm[h].last) fm[h].last=l.ts; if(l.ip) fm[h].ips.add(l.ip);
    });
    res.json({success:true,suspicious:Object.values(fm).filter(e=>e.count>=3).sort((a,b)=>b.count-a.count).map(e=>{
      const k=db.keys?.[e.kh]; return{key_hash:e.kh,key_display:k?.key||e.kh.slice(0,12)+'…',email:k?.email||'—',type:k?.type||'—',fail_count:e.count,last_attempt:e.last,ips:[...e.ips].join(', ')};
    })});
  } catch(e) { res.status(500).json({success:false,error:e.message}); }
});
app.get('/admin/logs', adminAuth, (req,res) => {
  try {
    const db=loadDB(), limit=Math.min(parseInt(req.query.limit)||200,1000);
    let logs=db.logs||[]; if(req.query.action) logs=logs.filter(l=>l.action===req.query.action);
    res.json({success:true,logs:logs.slice(0,limit),count:Math.min(logs.length,limit)});
  } catch(e) { res.status(500).json({success:false,error:e.message}); }
});
app.get('/admin/export', adminAuth, (req,res) => {
  const db=loadDB();
  res.setHeader('Content-Disposition','attachment; filename="medishop-export-'+Date.now()+'.json"');
  res.json({exportedAt:new Date().toISOString(),application:'MediShop License Server v4.0',...db});
});
...
app.get('/', (req,res) => {
  ...
  if (tok !== API_SECRET && tok !== ADMIN_KEY) {
    return res.status(401).send('<!DOCTYPE html><html><head><title>MediShop</title><meta charset="UTF-8"/>'
      ...
      +'<body><div style="font-size:60px">💊</div><h2>MediShop License Admin</h2>'
      +'<p style="color:#6b8ab0">URL: <code>/?token=YOUR_API_SECRET</code></p></body></html>');
  }

  // Build dashboard - inject secrets server-side
  const secret = API_SECRET;
  const trialD = TRIAL_DAYS;
  const maxD   = MAX_DEV;
...
.kv{font-family:monospace;font-size:17px;font-weight:700;color:#00c9a7;letter-spacing:.05em;word-break:break-all}
...
.vlb{font-size:10px;color:#4a6080;text-transform:uppercase}
.vvl{font-weight:700;font-size:13px}
</style></head>
<body>
<div class="bar">
  <div style="display:flex;align-items:center">
    ...
    <span class="bs">Cloud Sprint | contact@cloudsprint.in | 9985223448</span>
  </div>
  <div class="br">
    ...
    <button class="btn brd" onclick="location.href='/'" style="font-size:11px;padding:5px 10px">🚪 Sign Out</button>
  </div>
</div>
<div class="wrap">
  <div id="stats" class="stats">
    <div class="stat" style="border-top-color:#00c9a7;flex:1">
      <div class="sl"><span class="sp"></span>Loading stats…</div>
      <div class="sv" style="color:#4a6080">—</div>
    </div>
  </div>
  <div class="tabs">
    ...
    <div class="tab"    onclick="goTab('log')">📋 Activity Logs</div>
  </div>

  <div id="p-all" class="pane on">
    <div class="card">
      <div class="ch">
        <span class="ct">📋 All License Keys</span>
        <button class="btn bd" onclick="loadKeys()" style="font-size:11px;padding:5px 10px">↻ Refresh</button>
      </div>
      <div style="overflow-x:auto">
        <table><thead><tr>
          <th>License Key</th><th>Email</th><th>Type</th>
          <th>Devices</th><th>Status</th><th>Issued</th><th>Actions</th>
        </tr></thead>
        <tbody id="kb">
          <tr><td colspan="7" style="text-align:center;padding:30px;color:#4a6080"><span class="sp"></span>Loading…</td></tr>
        </tbody></table>
      </div>
    </div>
  </div>
    ...
    <div class="card">
      ...
      <div class="row">
        ...
        <select id="gTy">
          ...
          <option value="trial">Trial (${trialD} days)</option>
        </select>
        <input type="number" id="gDv" value="${maxD}" min="1" max="10" style="width:65px"/>
        <input type="text" id="gNt" placeholder="Notes (shop name…)" style="min-width:180px"/>
        <button class="btn bg" id="gBt" onclick="doGen()">✨ Generate &amp; Register</button>
      </div>
      <div id="gAl" class="al"></div>
      <div class="kbox" id="kbx">
        <div class="kv" id="ktx"></div>
        <div id="kmt" style="font-size:11px;color:#4a6080;margin-top:5px"></div>
        <div style="display:flex;gap:8px;margin-top:10px">
          <button class="btn bd" style="flex:1" onclick="doCopy()">📋 Copy</button>
          <button class="btn bg" style="flex:1" onclick="doWA()">📱 WhatsApp</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="p-imp" class="pane">
    <div class="card">
      ...
      <div class="row">
        ...
        <button class="btn bb" id="iBt" onclick="doImp()">📥 Register Key</button>
      </div>
  ...
  <div id="p-ver" class="pane">
    <div class="card">
      ...
      <div class="row">
        <input type="text" id="vKy" placeholder="Enter license key…" style="min-width:300px;font-family:monospace"/>
        <button class="btn bb" id="vBt" onclick="doVerify()">🔍 Verify</button>
      </div>
      <div id="vRes" class="vbox"></div>
    </div>
    <div class="card">
      <div class="ch"><span class="ct">🔄 Transfer to New Device</span></div>
      <p style="font-size:12px;color:#4a6080;margin-bottom:12px">Frees all device slots so customer can activate on a new device.</p>
      <div class="row">
        <input type="text" id="tKy" placeholder="License key…" style="min-width:300px;font-family:monospace"/>
        <button class="btn bo" onclick="doTransfer()">🔄 Free All Devices</button>
      </div>
      <div id="tAl" class="al"></div>
    </div>
    <div class="card">
      <div class="ch"><span class="ct">🚫 Revoke License</span></div>
      <p style="font-size:12px;color:#4a6080;margin-bottom:12px">Blocks this license — customer loses access immediately.</p>
      <div class="row">
        <input type="text" id="rKy" placeholder="License key…" style="min-width:300px;font-family:monospace"/>
        <button class="btn brd" onclick="doRevoke()">🚫 Revoke License</button>
      </div>
  ...
  <div id="p-sus" class="pane">
    <div class="card">
      <div class="ch">
        <span class="ct">⚠ Suspicious Activity</span>
        <button class="btn bd" onclick="loadSus()" style="font-size:11px;padding:5px 10px">↻ Refresh</button>
      </div>
      <div style="overflow-x:auto">
        <table><thead><tr><th>Key</th><th>Email</th><th>Type</th><th>Fails</th><th>Last Attempt</th><th>IPs</th></tr></thead>
        <tbody id="sb"><tr><td colspan="6" style="text-align:center;padding:20px;color:#4a6080">Click Refresh to load</td></tr></tbody>
        </table>
      </div>
    </div>
  </div>
  ...
  <div id="p-log" class="pane">
    <div class="card">
      <div class="ch">
        <span class="ct">📋 Activity Logs</span>
        <button class="btn bd" onclick="loadLogs()" style="font-size:11px;padding:5px 10px">↻ Refresh</button>
      </div>
      <div style="overflow-x:auto">
        <table><thead><tr><th>Time</th><th>Action</th><th>Result</th><th>Key</th><th>Device</th><th>IP</th><th>Detail</th></tr></thead>
        <tbody id="lb"><tr><td colspan="7" style="text-align:center;padding:20px;color:#4a6080">Click Refresh to load</td></tr></tbody>
        </table>
      </div>
    </div>
  </div>
...
function apiFetch(method, path, body, cb) {
  ...
  xhr.timeout = 15000;
  xhr.ontimeout = function(){ cb({success:false, error:'Request timed out. Server may be waking up — wait 30s and try again.'}); };
  xhr.onerror = function(){ cb({success:false, error:'Network error — check your connection.'}); };
  xhr.onload = function(){
    try { cb(JSON.parse(xhr.responseText)); }
    catch(e){ cb({success:false, error:'Bad response: '+xhr.responseText.slice(0,100)}); }
  };
...
function loadStats() {
  apiFetch('GET', '/dashboard', null, function(r) {
    if(!r.success) {
      document.getElementById('stats').innerHTML = '<div class="stat" style="border-top-color:#ef4444;flex:1"><div class="sl">Error loading stats</div><div class="sv" style="color:#ef4444;font-size:12px">'+(r.error||'Unknown error')+'</div></div>';
      return;
    }
    var s = r.stats || {};
    var items = [['Total Keys',s.totalKeys||0,'#00c9a7'],['Full',s.fullKeys||0,'#22c55e'],['Trial',s.trialKeys||0,'#f59e0b'],['Active Devices',s.totalDevices||0,'#0ea5e9'],["Today's Acts",r.todayActivations||0,'#a78bfa']];
    document.getElementById('stats').innerHTML = items.map(function(x){ return '<div class="stat" style="border-top-color:'+x[2]+'"><div class="sl">'+x[0]+'</div><div class="sv" style="color:'+x[2]+'">'+x[1]+'</div></div>'; }).join('');
  });
}
...
function loadKeys() {
  ...
  apiFetch('GET', '/keys', null, function(r) {
    ...
    document.getElementById('kb').innerHTML = r.keys.map(function(k) {
      ...
      return '<tr>'
        ...
        +'<td style="font-size:11px">'+k.email+(k.notes?'<br><span style="color:#4a6080">'+k.notes+'</span>':'')+'</td>'
        +'<td>'+tb+'</td>'
        +'<td style="text-align:center;font-weight:700">'+(k.active_devices||0)+'/'+k.max_devices+'</td>'
        +'<td>'+sb+'</td>'
        +'<td style="font-size:11px;color:#4a6080">'+fd(k.issued_at)+'</td>'
        +'<td><div style="display:flex;gap:4px;flex-wrap:wrap">'
        +(k.is_active
          ? '<button class="btn brd" style="font-size:10px;padding:3px 7px" onclick="doRvk(\''+k.key_hash+'\')">Revoke</button>'
          : '<button class="btn bg"  style="font-size:10px;padding:3px 7px" onclick="doRst(\''+k.key_hash+'\')">Restore</button>')
        +'<button class="btn bo" style="font-size:10px;padding:3px 7px" onclick="doFsl(\''+k.key_hash+'\')">Free Slots</button>'
        +'</div></td></tr>';
    }).join('');
  });
}
...
function doGen() {
  ...
  if(!em){a.textContent='Email required.';a.className='al er';return;}
  var bt=document.getElementById('gBt'); bt.disabled=true; bt.innerHTML='<span class="sp"></span>Generating…';
  apiFetch('POST','/keys/generate',{email:em,type:document.getElementById('gTy').value,maxDevices:document.getElementById('gDv').value,notes:document.getElementById('gNt').value.trim()},function(r){
    bt.disabled=false; bt.textContent='✨ Generate & Register';
    if(!r.success){a.textContent='Error: '+r.error;a.className='al er';return;}
    document.getElementById('ktx').textContent=r.key;
    document.getElementById('kmt').textContent=r.type.toUpperCase()+' | '+r.email+' | '+r.maxDevices+' device(s)';
    document.getElementById('kbx').style.display='block';
    a.textContent='Key generated successfully!'; a.className='al ok';
    loadKeys(); loadStats();
  });
}
function doCopy(){ navigator.clipboard.writeText(document.getElementById('ktx').textContent).then(function(){alert('Copied to clipboard!');}); }
function doWA(){ var k=document.getElementById('ktx').textContent; window.open('https://wa.me/?text='+encodeURIComponent('Your MediShop Pro license key:\n\n*'+k+'*\n\nActivate in the app under License Setup.\nSupport: 9985223448'),'_blank'); }
...
function doImp() {
  ...
  if(!ky){a.textContent='License key required.';a.className='al er';return;}
  if(!em){a.textContent='Email required.';a.className='al er';return;}
  var bt=document.getElementById('iBt'); bt.disabled=true; bt.innerHTML='<span class="sp"></span>Registering…';
  apiFetch('POST','/keys/import',{licenseKey:ky,email:em,type:document.getElementById('iTy').value,notes:document.getElementById('iNt').value.trim()},function(r){
    bt.disabled=false; bt.textContent='📥 Register Key';
    if(!r.success){a.textContent='Error: '+r.error;a.className='al er';return;}
    a.textContent='Key registered! Customer can now activate.'; a.className='al ok';
    document.getElementById('iKy').value=''; document.getElementById('iEm').value='';
    loadKeys(); loadStats();
  });
}
...
function doVerify() {
  ...
  var bt=document.getElementById('vBt'); bt.disabled=true; bt.innerHTML='<span class="sp"></span>Verifying…';
  apiFetch('POST','/verify',{licenseKey:ky},function(r){
    ...
    var dl=r.type==='full'?'Never expires':(r.daysLeft<=0?'<span style="color:#ef4444">EXPIRED</span>':r.daysLeft+' days remaining');
    var drows=(r.devices||[]).map(function(dv){ return '<tr><td>'+dv.deviceName+'</td><td class="mono" style="font-size:10px">'+dv.deviceId.slice(0,20)+'</td><td>'+fdt(dv.activatedAt)+'</td><td>'+fdt(dv.lastSeen)+'</td><td>'+(dv.isRevoked?'<span style="color:#ef4444">Revoked</span>':'<span style="color:#22c55e">Active</span>')+'</td></tr>'; }).join('');
    ...
    d.innerHTML='<div class="vrow"><span class="vlb">Status</span><span class="vvl">'+(r.isActive?'<span style="color:#22c55e">VALID '+r.type.toUpperCase()+'</span>':'<span style="color:#ef4444">REVOKED</span>')+'</span></div>'
      ...
      +(drows?'<div style="margin-top:10px"><table style="width:100%;font-size:11px;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:4px;color:#4a6080;font-size:10px;border-bottom:1px solid #162035">Device</th><th style="text-align:left;padding:4px;color:#4a6080;font-size:10px;border-bottom:1px solid #162035">ID</th><th style="text-align:left;padding:4px;color:#4a6080;font-size:10px;border-bottom:1px solid #162035">Activated</th><th style="text-align:left;padding:4px;color:#4a6080;font-size:10px;border-bottom:1px solid #162035">Last Seen</th><th style="text-align:left;padding:4px;color:#4a6080;font-size:10px;border-bottom:1px solid #162035">Status</th></tr></thead><tbody>'+drows+'</tbody></table></div>':'');
  });
}

function doTransfer() {
  var ky=document.getElementById('tKy').value.trim(), a=document.getElementById('tAl'); a.className='al';
  if(!ky){a.textContent='Enter a license key.';a.className='al er';return;}
  if(!confirm('Free all device slots? Customer can activate on new device.')) return;
  apiFetch('POST','/verify',{licenseKey:ky},function(rv){
    if(!rv.success){a.textContent='Error: '+rv.error;a.className='al er';return;}
    apiFetch('POST','/devices/'+rv.keyHash+'/transfer-all',null,function(r){
      if(r.success){a.textContent='All slots freed. Customer can activate on new device.';a.className='al ok';loadKeys();}
      else{a.textContent='Error: '+r.error;a.className='al er';}
    });
  });
}

function doRevoke() {
  var ky=document.getElementById('rKy').value.trim(), a=document.getElementById('rAl'); a.className='al';
  if(!ky){a.textContent='Enter a license key.';a.className='al er';return;}
  if(!confirm('REVOKE this license? Customer will lose access.')) return;
  apiFetch('POST','/verify',{licenseKey:ky},function(rv){
    if(!rv.success){a.textContent='Error: '+rv.error;a.className='al er';return;}
    apiFetch('POST','/keys/'+rv.keyHash+'/revoke',null,function(r){
      if(r.success){a.textContent='License revoked.';a.className='al ok';loadKeys();}
      else{a.textContent='Error: '+r.error;a.className='al er';}
    });
  });
}

function doRvk(h){ if(!confirm('Revoke this license?')) return; apiFetch('POST','/keys/'+h+'/revoke',null,function(r){if(r.success)loadKeys();}); }
function doRst(h){ apiFetch('POST','/keys/'+h+'/restore',null,function(r){if(r.success)loadKeys();}); }
function doFsl(h){ if(!confirm('Free all device slots?')) return; apiFetch('POST','/devices/'+h+'/transfer-all',null,function(r){if(r.success)loadKeys();}); }
...
function loadSus() {
  ...
  apiFetch('GET','/suspicious',null,function(r){
...
function loadLogs() {
  ...
  apiFetch('GET','/logs',null,function(r){
    if(!r.success||!r.logs||!r.logs.length){document.getElementById('lb').innerHTML='<tr><td colspan="7" style="text-align:center;padding:20px;color:#4a6080">No logs yet</td></tr>';return;}
    var colors={'ok':'#22c55e','reactivate':'#00c9a7'};
    document.getElementById('lb').innerHTML=r.logs.map(function(l){return '<tr><td style="font-size:10px;color:#4a6080;white-space:nowrap">'+fdt(l.ts)+'</td><td style="font-weight:700;color:#7dd3fc">'+l.action+'</td><td style="color:'+(colors[l.result]||'#ef4444')+'">'+l.result+'</td><td class="mono" style="font-size:10px">'+(l.kh||'').slice(0,10)+'…</td><td style="font-size:11px">'+(l.did||'').slice(0,18)+'</td><td style="font-size:11px;color:#4a6080">'+(l.ip||'')+'</td><td style="font-size:11px">'+(l.detail||'')+'</td></tr>';}).join('');
  });
}
